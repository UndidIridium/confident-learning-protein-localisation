...
| Baseline — same features, no cleaning | ~0.767 |
...
training set (13,465 → 7,111 proteins). The measured estimate of true label noise is **37.6%** —
cleanlab finds the noise rather than guessing it. Cytoplasm is the noisiest compartment (est.
...
**The cutoff is tuned, not assumed.** Sweeping the cleanlab cutoff gives a clear inverted U
({0.40: 0.7994, 0.45: 0.7985, 0.50: 0.8002, 0.55: 0.7968}) — 0.50 is optimal, not 0.40.

**Cleaning and features compound.** SPACE alone +0.017, cleaning alone +0.029, both together
+0.066 — more than the sum of the parts (+0.020 synergy): network features make the OOF model
confident enough for cleanlab to spot real mislabels.
...
├── figures/                  # dissertation figures (PNG)
...
Measured mean: **0.7838 ± 0.0156** (CL_CUTOFF=0.40). To reproduce the 0.50
configuration across folds, set `CL_CUTOFF = 0.50` on line 34 and rerun.
...
working directory (the "tinker" tree) where every idea gets a script; anything that proves out
gets **promoted** into this repo. Full history is in `docs/EXPERIMENT_LOG.md` and
